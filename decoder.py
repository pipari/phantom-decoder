...
class PositionFrame(Frame):
    ...
    def __repr__(self):
        return "<PositionFrame: (%.05f, %.05f), %4.01f m, %s, %s, %s, %s, %s, %s, %s, %s>" % (
            self.latitude,
            self.longitude,
            self.ascent,
            self.x_speed,
...
class TimeFrame(Frame):
    ...
    def __repr__(self):
        return "<TimeFrame: %.02f m/s, %.01f m, %s>" % (
            self.speed,
            self.distance,
            self.timestamp,
        )
...
class ControllerFrame(Frame):
    ...
    def __repr__(self):
        return "<ControllerFrame: %.02f, %.02f, %.02f, %.02f>" % (
            self.throttle,
            self.rudder,
            self.elevator,
            self.aileron,
...
class GimbalFrame(Frame):
    def __init__(self, raw_frame):
        ...
        (
            ...
            self.__version,
            self.__counter,
        ) = struct.unpack_from("<hhhBBBBBBI", self.body, 0)
...
class BatteryFrame(Frame):
    def __init__(self, raw_frame):
        ...
        (
            ...
            _date,
            temperature,
        ) = struct.unpack_from("<BHHHBIHhHHHHHHHHH", self.body, 0)

        self.current_pv = current_pv / 1000.0
        self.current = current / 1000.0
        self.voltages = [
            voltage_cell_1 / 1000.0,
            voltage_cell_2 / 1000.0,
            voltage_cell_3 / 1000.0,
            voltage_cell_4 / 1000.0,
            voltage_cell_5 / 1000.0,
            voltage_cell_6 / 1000.0,
        ]
        self.manufacture_date = datetime.date(
            (self.body[-4] >> 1) + 1980,
            ((self.body[-4] & 0x1) << 3) | (self.body[-5] >> 5),
            self.body[-5] & 0x1F,
        )
        self.temperature = temperature / 10 - 273.15

    def __repr__(self):
        return "<BatteryFrame: %s %s %s %s %s %s %s %s %s %s %s %s>" % (
            self.percent,
            self.current_pv,
            self.current_capacity,
            self.total_capacity,
...
class SmartBatteryFrame(Frame):
    ...
    def __repr__(self):
        return "<SmartBatteryFrame: %s %s %s %s %s %s %s %s>" % (
            self.useful_time,
            self.go_home_time,
            self.land_time,
            self.go_home_battery,
...
class MessageFrame(Frame):
    def __init__(self, raw_frame):
        ...
        self.message = struct.unpack("%ss" % len(self.body), self.body)[0]
...
class AircraftFrame(Frame):
    def __init__(self, raw_frame):
        ...
        self.aircraft_name = aircraft_name.decode("utf-8").replace("\0", "")
        self.active_timestamp = time.ctime(active_timestamp)

    def __repr__(self):
        return "<AircraftFrame: %s %s %s %s %s %s %s %s %s>" % (
            self.drone_type,
            self.app_type,
            self.app_version,
            self.aircraft_serial_no,
...
class EmptyFrame(Frame):
    def __init__(self, **kwargs):
        super(EmptyFrame, self).__init__({"body": None, "type": None})
...
def decode_file(path):
    ...
    while i < len(body):
        ...
        f = {"type": frame_type, "body": frame_body}
...
if __name__ == "__main__":
    path = "tests/DJIFlightRecord_2022-08-18_[13-55-33].txt"
